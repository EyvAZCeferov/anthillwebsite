@if (auth()->check())
    <div class="left_profile_tab">
        ...
        @if (auth()->user()->type == 3)
            <div class="element @yield('addservice_menu') " onclick="window.open(`{{ route('services.addnew') }}`)">
                <i class="las la-plus-circle"></i> <span>@lang('additional.urls.addservice')</span>
            </div>
        @endif
        <div class="element @yield('orders_menu')" onclick="window.open(`{{ route('orders.index') }}`)">
        ...
        <div class="element @yield('payments_menu')" onclick="window.open(`{{ route('payments.index') }}`)">
            <i class="las la-file-invoice-dollar"></i> <span>@lang('additional.urls.payments')</span>
        </div>
        @if (auth()->user()->type == 3)
            <div class="element @yield('myservices_menu')" onclick="window.open(`{{ route('myservices.index') }}`)">
                <i class="las la-cube"></i> <span>@lang('additional.urls.myservices')</span>
            </div>
        @endif
        <hr>

        <div class="element @yield('settings_menu')" onclick="window.open(`{{ route('settings.index') }}`)">
            <i class="las la-cog"></i> <span>@lang('additional.urls.settings')</span>
        </div>
        <div class="element @yield('logout_menu')" onclick="window.open(`{{ route('logout.index') }}`)">
            <i class="las la-sign-out-alt"></i> <span>@lang('additional.urls.logout')</span>
        </div>
        <div class="element profile @yield('profile_menu')" onclick="window.open(`{{ route('auth.profile') }}`)">
