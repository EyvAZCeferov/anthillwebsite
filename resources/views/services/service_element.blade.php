@if (
    $data->user->type == 3 &&
        !empty($data->user->additionalinfo->company_slugs) &&
        isset($data->user->additionalinfo->company_slugs[app()->getLocale() . '_slug']))
    <div class="service_one column-28 mobile_column-45" id="service-{{ $data->code }}">
        @if (isset($data->user) &&
            ...
            <div class="company_area"
                ...
                <img class="lazyload blur-up"
                    data-src="{{ App\Helpers\Helper::getImageUrl($data->user->additionalinfo->company_image, 'users') }}" />
            </div>
        @endif
        @if (auth()->check() && isset($ty))
            @if (auth()->user()->id == $data->user_id)
                <div class="more">
                    <span class="more_button" onclick="toggledropdowforcreator(`{{ $data->code }}`)"><i
                            class="las la-ellipsis-v"></i></span>
                    <div class="dropdown">
                        <div class="dropdown_element"
                            onclick="deletequeryservice(`{{ $data->code }}`,`{{ $data->user_id }}`,'@lang('additional.messages.areyousuredelete')','{{ app()->getLocale() }}')">
                            <i class="las la-trash"></i> @lang('additional.buttons.delete')
                        </div>
                        <div class="dropdown_element"
                            onclick="window.location.href=`{{ route('services.edit', ['code' => $data->code]) }}`">
                            <i class="las la-undo-alt"></i> @lang('additional.buttons.update')
                        </div>
                        <div class="dropdown_element"
                            onclick="createpaymentlink(`{{ $data->code }}`,'{{ auth()->user()->id }}','{{ app()->getLocale() }}',false)">
            ...
            @else
                <div class="bookmark @if (auth()->check() && !empty(wishlist_items(auth()->id(), $data->id))) active @endif"
                    ...
                    <i class="@if (auth()->check() && !empty(wishlist_items(auth()->id(), $data->id))) fas fa-bookmark @else las la-bookmark @endif "></i>
                </div>
            @endif
        @else
            <div class="bookmark @if (auth()->check() && !empty(wishlist_items(auth()->id(), $data->id))) active @endif"
        ...
        @if (isset($data->images[0]) && !empty($data->images[0]))
            <div class="image_area"
                ...
                <img class="lazyload blur-up"
                    data-src="{{ App\Helpers\Helper::getImageUrl($data->images[0]->original_images, 'products') }}"
        ...
        <p class="price"
            onclick="window.location.href=`{{ route('services.show', $data->slugs[app()->getLocale() . '_slug']) }}`)">
            <span style="font-size:12px;color:gray;font-weight:bold">
                @if (!empty(lang_properties('startsat', 'keyword')))
                    {{ lang_properties('startsat', 'keyword')->name }}
                @else
                    Starts at
                @endif
                </spn>
